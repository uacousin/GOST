...
namespace GOST
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string M = "Here is some data to encrypt!";

            // Create a new instance of the TripleDESCryptoServiceProvider
            // class.  This generates a new key and initialization 
            // vector (IV).
            TripleDES myTripleDES = TripleDES.Create();

            // Encrypt the string to an array of bytes.
            byte[] encrypted = TripleDESManaged.EncryptStringToBytes(M, myTripleDES.Key, myTripleDES.IV);

            // Decrypt the bytes to a string.
          //  string roundtrip = TripleDESManaged.DecryptStringFromBytes(encrypted, myTripleDES.Key, myTripleDES.IV);
           ...
           // Console.WriteLine("Round Trip: {0}", roundtrip);
            ...
            string encoded = Encoding.Unicode.GetString(encrypted);

            //Calculating hash of M

            Console.WriteLine("\nCalculating h({0})", encoded);
            hashOfM = Hash(encoded);
         ...
         /*   signature.hashOfM = Hash("message2");
            ...
            string roundtrip = TripleDESManaged.DecryptStringFromBytes(Encoding.Unicode.GetBytes(encoded), myTripleDES.Key, myTripleDES.IV);
            Console.WriteLine("Round Trip: {0}", roundtrip);
